...
assert.match(app, /quotDateFilter|selectedDate|fallbackDate|find.*date.*quotation/i);
...
assert.match(app, /function clearLegacyBrowserState\(/);
assert.match(app, /clearLegacyBrowserState\(\);/);
assert.match(app, /selectedQuotationDate\s*=\s*document\.getElementById\('quotDateFilter'\)\?\.value \|\| todayISO\(\);/);
assert.match(app, /date:\s*importedDate\s*\|\|\s*selectedQuotationDate/);
assert.match(app, /Notifications/);
assert.match(html, /id="quotDateFilter"/);
